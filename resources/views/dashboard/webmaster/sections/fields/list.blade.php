@if (count($WebmasterSections->allCustomFields) > 0)
    <div class="row p-a">
        <a class="btn btn-fw primary" href="{{ route('webmasterFieldsCreate', [$WebmasterSections->id]) }}">
...
@if (count($WebmasterSections->allCustomFields) > 0)
        ...
        <table class="table table-bordered">
            <thead class="dker">
                <tr>
                    ...
                    <th class="text-center" style="width:120px;">{{ __('backend.customFieldsRequired') }}</th>
                    <th class="text-center" style="width:100px;">{{ __('backend.language') }}</th>
                ...
                @foreach ($WebmasterSections->allCustomFields as $customField)
                    ...
                    <tr>
                        <td class="{!! $customField->css_class !!} dker"><label class="ui-check m-a-0">
                                <input type="checkbox" name="ids[]" value="{{ $customField->id }}"><i
                                    class="dark-white"></i>
                                {!! Form::hidden('row_ids[]', $customField->id, ['class' => 'form-control row_no']) !!}
                            </label>
                        </td>

                        <td class="h6 {!! $customField->css_class !!}">
                            {!! Form::text('row_no_' . $customField->id, $customField->row_no, [
                        ...
                        <td class="text-center {!! $customField->css_class !!}">
                            <i
                                class="fa {{ $customField->status == 1 ? 'fa-check text-success' : 'fa-times text-danger' }} inline"></i>
                        </td>
                        <td class="text-center {!! $customField->css_class !!}">
                            ...
                            @if (@Auth::user()->permissionsGroup->delete_status)
                                <button class="btn btn-sm warning" data-toggle="modal"
                                    data-target="#mf-{{ $customField->id }}" ui-toggle-class="bounce"
                                    ui-target="#animate">
                                    <small><i class="material-icons">
                                            &#xe872;</i> {{ __('backend.delete') }}
                                    </small>
    ...
    <div class="row">
        <div class="col-sm-3 hidden-xs">
            ...
            <select name="action" id="action4" class="form-control c-select w-sm inline v-middle" required>
            ...
            <button type="submit" id="submit_all4" class="btn white">{{ __('backend.apply') }}</button>
            <button id="submit_show_msg4" class="btn white" data-toggle="modal" style="display: none"
                data-target="#mf-all" ui-toggle-class="bounce" ui-target="#animate">{{ __('backend.apply') }}
            </button>
        </div>
    </div>
    {{ Form::close() }}
