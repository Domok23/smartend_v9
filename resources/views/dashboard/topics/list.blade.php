            ...
            <div>
                ...
                icons: {
                  ...
                  close: 'fa fa-remove'
                                                ...
                                                {!! Form::text('date', @$_GET['date'] != '' ? Helper::formatDate(@$_GET['date']) : '', [
                                                    'placeholder' => __('backend.topicDate'),
                                                    'class' => 'form-control',
                                                    'id' => 'find_date',
                                                ]) !!}
                                                    ...
                                                    <select name="{{ 'customField_' . $customField->id }}"
                                                        ...
                                                        @foreach ($cf_details_lines as $cf_details_line)
                                                            <option value="{{ $line_num }}"
                                                                {{ @$_GET['customField_' . $customField->id] == $line_num ? "selected='selected'" : '' }}>
                ...
                icons: {
                  ...
                  close: 'fa fa-remove'
                                                            ...
                                                            {!! Form::text(
                                                                ...
                                                                @$_GET['customField_' . $customField->id] != ''
                                                                    ? Helper::formatDate(@$_GET['customField_' . $customField->id])
                                                                    : '',
                                                                [
                                                                    'placeholder' => $cf_title,
                                                                    'class' => 'form-control',
                                                                    'id' => 'customField_' . $customField->id,
                                                                    $cf_required => '',
                                                                    'dir' => $cf_land_dir,
                                            ...
                                            @elseif($customField->type == 3)
                                                <div class="col-md-3 col-xs-6 m-b-5p">
                                                    {!! Form::email('customField_' . $customField->id, @$_GET['customField_' . $customField->id], [
                                                        'placeholder' => $cf_title,
                                                        'class' => 'form-control',
                                                        'id' => 'customField_' . $customField->id,
                                                        $cf_required => '',
                                            ...
                                            @elseif($customField->type == 2)
                                                <div class="col-md-3 col-xs-6 m-b-5p">
                                                    {!! Form::number('customField_' . $customField->id, @$_GET['customField_' . $customField->id], [
                                                        'placeholder' => $cf_title,
                                                        'class' => 'form-control',
                                                        'id' => 'customField_' . $customField->id,
                                                        $cf_required => '',
                                            ...
                                            @else
                                                <div class="col-md-3 col-xs-6 m-b-5p">
                                                    {!! Form::text('customField_' . $customField->id, @$_GET['customField_' . $customField->id], [
                                                        'placeholder' => $cf_title,
                                                        'class' => 'form-control',
                                                        'id' => 'customField_' . $customField->id,
                                                        $cf_required => '',
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" style="width: 100%" id="topics_{{ $WebmasterSection->id }}">
                        <thead class="dker">
                        ...
                        <div class="col-sm-12 hidden-xs">
                            ...
                            @if (@Auth::user()->permissionsGroup->edit_status)
                                <select name="action" id="action"
                                    class="input-sm form-control w-sm inline v-middle c-select" required>
                    ...
                    new Chart(document.getElementById('chart-area-{{ $customField->id }}').getContext('2d'), {
                        ...
                        data: {
                            ...
                            labels: [
                                ...
                                ?>
                                @foreach ($cf_details_lines as $cf_details_line)
                                    ('{{ $cf_details_line }}').substring(0, 40) + (((
                                            '{{ $cf_details_line }}').length > 40) ? '..' : '') + " ( " +
                                        '{{ @$statics[$customField->id][$line_num] != '' ? @$statics[$customField->id][$line_num] : 0 }}' +
                                        ' )',
                                        <?php
                                        $line_num++;
                        ...
                        @foreach ($WebmasterSection->customFields as $customField)
                            @if ($customField->in_search)
                                data.customField_{{ $customField->id }} = $(
                                    '#customField_{{ $customField->id }}').val();
                            @endif
                        @endforeach
                    }
                ...
                "columns": [
                    ...
                    @foreach ($WebmasterSection->customFields as $customField)
                        ...
                        @if ($customField->in_table)
                            @if ($customField->lang_code == 'all' || $customField->lang_code == @Helper::currentLanguage()->code)
                                {
                                    "data": "field_{{ $customField->id }}",
                                    "orderable": false,
                                    "createdCell": function(td, cellData, rowData, row, col) {
                                        $(td).addClass(rowData[
                                            'class_field_{{ $customField->id }}']);
                                    },
                                },
                            @endif
                        @endif
