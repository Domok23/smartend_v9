...
@section('content')
    <div class="padding">
        ...
        {{ Form::close() }}
        <p class="m-b-md">
            <strong>{{ $totalcount = count($Topics) + count($Sections) + count($Contacts) + count($Events) + count($Webmails) }}</strong>
            {{ __('backend.resultsFoundFor') }}
        ...
        <ul class="nav nav-sm nav-pills nav-active-primary clearfix">
            @if (count($Topics) > 0)
                <li class="nav-item">
                    <a class="nav-link {{ $active_tab == 1 ? 'active' : '' }}" href data-toggle="tab" data-target="#tab_1">
                        {{ __('backend.pages') }}
                        <span class="label label-xs primary m-l-xs">{{ count($Topics) }}</span></a>
                </li>
            @endif
            @if (count($Sections) > 0)
                <li class="nav-item">
                    <a class="nav-link {{ $active_tab == 2 ? 'active' : '' }}" href data-toggle="tab" data-target="#tab_2">
                        {{ __('backend.categories') }}
                        <span class="label label-xs primary m-l-xs">{{ count($Sections) }}</span></a>
                </li>
            @endif
            @if (count($Contacts) > 0)
                <li class="nav-item">
                    <a class="nav-link {{ $active_tab == 3 ? 'active' : '' }}" href data-toggle="tab" data-target="#tab_3">
                        {{ __('backend.newsletter') }}
                        <span class="label label-xs primary m-l-xs">{{ count($Contacts) }}</span></a>
                </li>
            @endif
            @if (count($Events) > 0)
                <li class="nav-item">
                    <a class="nav-link {{ $active_tab == 4 ? 'active' : '' }}" href data-toggle="tab" data-target="#tab_4">
                        {{ __('backend.notesEvents') }}
                        <span class="label label-xs primary m-l-xs">{{ count($Events) }}</span></a>
                </li>
            @endif
            @if (count($Webmails) > 0)
                <li class="nav-item">
                    <a class="nav-link {{ $active_tab == 5 ? 'active' : '' }}" href data-toggle="tab" data-target="#tab_5">
                        {{ __('backend.inbox') }} <span
                                            ...
                                            <span class="w-40 avatar">
                                                ...
                                                @else
                                                    <img src="{{ asset('uploads/contacts/profile.jpg') }}"
                                                        class="on b-white bottom" style="opacity: 0.5">
                                                @endif
                                            </span>
                                        </a>
