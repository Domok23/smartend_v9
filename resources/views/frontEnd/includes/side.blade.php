<div class="col-lg-4">
    <aside class="right-sidebar">
                ...
                <ul class="cat">
                    @foreach ($Categories as $Category)
                        ...
                        <li>
                            ...
                            @endif
                            <a {{ $active_cat }}
                                href="{{ Helper::categoryURL($Category->id) }}">{{ $Category_title }}</a><span
                                class="pull-right">({{ $ccount }})</span>
                        </li>
                        @foreach ($Category->fatherSections as $MnuCategory)
                            ...
                            <li> &nbsp; &nbsp; &nbsp;
                                ...
                                @endif
                                <a {{ $active_cat }}
                                    href="{{ Helper::categoryURL($MnuCategory->id) }}">{{ $MnuCategory_title }}</a><span
                                    class="pull-right">({{ $ccount }})</span>
                            </li>
                        @endforeach
                    @endforeach
                                        ...
                                        @elseif($TopicMostViewed->webmasterSection->type == 2 && $TopicMostViewed->video_file != '')
                                            ...
                                            @elseif($TopicMostViewed->video_type == 2)
                                                ...
                                                @if ($Vimeo_id != '')
                                                    ...
                                                    <img src="{{ $hash[0]['thumbnail_large'] }}" class="pull-left"
                                                        alt="{{ $side_title }}" />
                                                @endif
                                            @endif
                                        @endif
        ...
        @include('frontEnd.includes.banners', [
            'BannersSettingsId' => Helper::GeneralWebmasterSettings('side_banners_section_id'),
        ])
